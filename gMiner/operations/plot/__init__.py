...
gFeatMiner can generate different kind of plots. To access this functionality you must specify ``operation_type=plot`` in the job request as seen in this example, in addition to a few other parameters that are detailed below::
...
# Internal modules #
from gMiner.constants import gm_project_name
...
def run(request, tracks, output_dir):
    ...
    # Save it #
    if request.get('output_name'):
        path = output_dir + '/' + request['output_name'] + '.png'
    else:
        path = output_dir + '/gminer_' + plot_type + '.png'

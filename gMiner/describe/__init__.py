...
One of the primary goals of gFeatMiner is to provide simple graphs that help the user describe and explore his genomic datasets. These graphs are generated quickly and on-the-fly as they are requested. To access this functionality you must specify ``operation='describe'`` in the job request as seen in this example, in addition to a few other parameters that are detailed below::
...
**operation**          This field must be specified in the request and must take the value of ``describe`` if one wishes to perform descriptive statistic operations on the tracks provided.
...
**output_location**    Specifies the directory in which the newly created image will be written.
...
class gmSubtrack(object):
    ...
    def __iter__(self):
        if self.selection['type'] == 'chr':
            ...
            for chr in self.track: yield self.track.read(chr, self.fields)
...
def track_cut_down(request, track):
    ...
    if not regions:
        ...
        if not request['per_chromosome']:
           ...
           for chr in track: yield gmSubtrack(request, track, {'type': 'chr', 'chr': chr})
...
def gm_get_characteristic(subtrack, chara):
    ...
    if shortcutable: shortcut = False
...
def run(request, tracks, output_dir):
    ...
    # Optional 'selected_regions' parameter #
    request['selected_regions'] = request.get('selected_regions', '')
    if request['selected_regions'] and not os.path.exists(request['selected_regions']):
        try:
            X = request['selected_regions']
            X = [dict([['chr',p[0]],['start',int(p[1])],['end',int(p[2])]]) for p in [r.split(':') for r in X.split(';')]]
            request['selected_regions'] = X
        except (ValueError, IndexError):
            message = "The selected regions are not properly formated or the file doesn't exist: '%s'"
            raise Exception(message % X)
    # Optional 'per_chromosome' parameter #
    request['per_chromosome'] = request.get('per_chromosome', False)
    request['per_chromosome'] = bool(request['per_chromosome'])
    # Optional 'compare_parents' parameter #

"""
Methods that create the 8 different plots according to the type of statistic to represent.
"""
...
default_color_selection = 'green'
default_color_parent    = 'blue'
...
class gmGraph(object):
    ...
    def generate(self):
        ...
        self.canvas.set_window_title('(' + project_name + ') ' + self.graph_title)
        ...
        fig.text(leftcorner, topleftcorner, project_name + ' generated graph', horizontalalignment='left', fontsize=self.fontsize0)
    ...
    def gen_legend(self):
        if self.b_comp and not self.b_bar and (not self.b_chr or not self.b_many):
            self.graph_legend.append([default_color_parent , "Whole track"])
            self.graph_legend.append([default_color_selection , "Selection"])
        if self.b_many and self.b_chr:
            for track in self.tracks:
                self.graph_legend.append([number_to_color(track.number) , track.name])
        ...
        if self.b_chr:#      chr: -----YES-----
            ...
            for i, chr in enumerate(self.chrs):
                for j, track in enumerate(self.tracks):
                    ...
                    if self.b_many: tmp_color = number_to_color(track.number)
                    else: tmp_color = default_plot_color
...
class gmPlotElement(object):
    ...
    def __init__(self, request, subtracks, position, height, color=default_plot_color):
        self.request = request
        self.subtracks = subtracks
    ...
    def plot(self, axes):
        ...
        if len(self.subtracks) == 2:
            ...
            if stat_selection:
                ...
                if not self.request['per_chromosome'] or not 'track2' in self.request: self.color = default_color_selection
            ...
            if stat_parent:
                ...
                if not self.request['per_chromosome'] or not 'track2' in self.request: self.color = default_color_parent
